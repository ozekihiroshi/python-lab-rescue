...
TARGET = HERE / "inspect_water_points.py"
...
def test_sorted_copy():
    ...
    expected = records.sort_values(["facility_id", "date"]).reset_index(drop=True)
...
def test_other_data():
    sample = pd.DataFrame([
        {"date": "2026-01-02", "facility_id": "B", "district": " North "},
        {"date": "2026-01-01", "facility_id": "A", "district": "North"},
    ])
    view = module.build_key_date_view(sample)
    assert view["facility_id"].tolist() == ["A", "B"]
